...
- **`income.ts`** — `incomeSchedule`: taxable income, SG and salary sacrifice as a function of the month, from `Person.payEvents` and July-anchored `incomeGrowthRate`. Tax years in the engine are Australian FYs (`fyOf`), not plan years.
...
Single-page React app; the Plans view is in `App.tsx` (household form, scenario table, chart), the Tracking view in `Tracking.tsx`, shared form bits in `ui.tsx`. Talks to the API through `api.ts`. No router (a `#tracking` hash picks the view), no state library — local component state plus the API round-trip on save/run.
